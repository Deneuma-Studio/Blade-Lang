...
namespace blade
{
	...
	extern "C" struct BladeVariant
	{
		...
		BladeVariant() = default;
    	BladeVariant(const Variant& v) { memcpy(opaque, &v, GODOT_CPP_VARIANT_SIZE); }
	};
...
private:
	...
	static const char* CompilerIncludeHandler(void* opaque, const char* filename, size_t* len)
	{
		...
		const json_t bindingsDatabase = BladeLanguage::get_singleton()->GetBindingsDatabase();
...
void BladeLanguage::_init()
{
	...
	// Verbose Debug Mode
	#if DEV_BUILD
		Verbose("Runtime is built in Developer Mode.");
	#endif
...
bool BladeLanguage::CreateBindings()
{
	...
	try
	{
		json_t extensionAPI = json_t::parse(ReadStdStringFromFile(GetStdStr(extensionAPIPath)));
		json_t generatedDatabase;
		...
		auto GenEnums = [&](const json_t& cls)
		{
			std::string out;
		...
		auto GenHeader = [&](const std::string& name, const json_t& cls, bool isRef)
		{
		...
		auto GenGlobalEnums = [&](const json_t& api)
		{
			std::string out;
		...
		auto GenUtilityFunctions = [&](const json_t& api)
		{
			std::string out;
			...
			for (const auto& u : api["utility_functions"])
			{
				...
				const auto& args = has_args ? u.at("arguments") : json_t::array();
...
bool BladeLanguage::LoadBindingsDatabase()
{
	...
	try
	{
		bindingsDatabase = json_t::parse(GetStdStr(bindingsDatabaseContent));
		return true;
	}
...
const json_t BladeLanguage::GetBindingsDatabase() const
{
...
bool BladeScript::_has_method(const StringName& p_method) const
{
	...
	const json_t& funcs = GetScriptMetadata()["Functions"];
...
bool BladeScript::_has_script_signal(const StringName& p_signal) const
{
	...
	const json_t& sigs = GetScriptMetadata()["Signals"];
...
bool BladeScript::_has_property_default_value(const StringName& p_property) const
{
	...
	const json_t& props = GetScriptMetadata()["Properties"];
...
void BladeScript::BindScriptSymbols(BladeCompiler* compiler)
{
	...
	compiler->AddSymbol("__aeabi_memset", BLADEFUNCTION(&::memset));
	compiler->AddSymbol("__aeabi_memmove", BLADEFUNCTION(&::memmove));
}
String BladeScript::PreProcessScript(json_t& metadata)
{
	// Use Namespace
	using namespace blade::syntax;

	// Prepare Metadata Arrays
	json_t includes = json_t::array();
	json_t properties = json_t::array();
	json_t functions = json_t::array();
	json_t signals = json_t::array();
	json_t extensions = json_t::array();
	...
	auto GenerateProxies = [&]() -> String
	{
		...
		for (const auto& ext : exts)
		{
			...
			const json_t& params = ext["params"];
...
bool BladeScript::CompileScript(bool dryPass)
{
	...
	if (IsRuntimeExecution())
	{
		...
		try
		{
			newMetadata = json_t::parse(GetStdStr(ReadStringFromFile(cachePath)));
		}
		catch(const std::exception&)
		{
...
bool BladeScript::PostProcessScript(BladeCompiler* compiler, json_t& metadata)
{
	// Get Script Compiler
	...
	const json_t& props = metadata["Properties"];
	...
	const json_t& exts = metadata["Extensions"];
	for (const auto& ext : exts)
	{
		BladeExtension be;
...
void BladePlugin::_enter_tree()
{
	...
	if (!editor_theme->has_icon(blade::BladeScriptType, "EditorIcons"))
	{
		...
		Ref<ImageTexture> iconImage = CreateSVGFromByteArray(blade::resources::SVG_BLADE_SCRIPT_ICON, sizeof blade::resources::SVG_BLADE_SCRIPT_ICON, iconSize);
...
extern "C" BLADE_EXPORT GDExtensionBool InitializeLanguage(GDExtensionInterfaceGetProcAddress p_get_proc_address, const GDExtensionClassLibraryPtr p_library, GDExtensionInitialization* r_initialization)
{
	// Initailize Object & Solve API Functions
	GDExtensionBinding::InitObject init_obj(p_get_proc_address, p_library, r_initialization);
	init_obj.register_initializer(RegisterBlade);
	init_obj.register_terminator(UnregisterBlade);
