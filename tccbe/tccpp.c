...
ST_FUNC void preprocess(int is_bof)
{
 ...
 redo:
    ...
    case TOK_INCLUDE_NEXT:
        ...
        /* --- Include Callback Hook (Required for Blade VFS)--- */
        if (s1->include_func) {
            size_t mem_len = 0;
            const char* mem_buf = s1->include_func(s1->include_opaque, buf, &mem_len);

            if (mem_buf)
            {
                tcc_open_bf(s1, buf, mem_len);
                memcpy(file->buffer, mem_buf, mem_len);
                if (s1->include_stack_ptr >= s1->include_stack + INCLUDE_STACK_SIZE) tcc_error("#include recursion too deep");
