...
LIBTCCAPI const char* tcc_preprocess_source(TCCState* s, const char* src)
{
    int old_output_type = s->output_type;
    FILE* old_ppfp = s->ppfp;
    ...
    int ret = tcc_compile_string(s, src);
    ...
    char* buf = (char*)tcc_malloc((size_t)size + 1);
    if (!buf) {
        fclose(tmp);
        return NULL;
    }
...
/* add custom include callback */
LIBTCCAPI void tcc_set_include_func( TCCState* s, void* opaque, const char* (*func)(void* opaque, const char* filename, size_t* len))
{
    s->include_opaque = opaque;
    s->include_func = func;
}
